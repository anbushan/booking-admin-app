...
export default function LoginPasscodeScreen({ navigation }: any) {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      showSuccess(t("loginPasscode.passcodeDownloaded"));
    } catch (err: any) {
  ...
  async function handleRevoke() {
    if (!window.confirm(t("loginPasscode.webRevokeConfirm"))) return;
    setBusy(true);
    try {
...
const styles = StyleSheet.create({
  ...
  title: { ...typography.titleCompact },
  subtitle: { ...typography.small, color: colors.textMuted, marginTop: spacing.xs, marginBottom: spacing.md, lineHeight: 18 },
