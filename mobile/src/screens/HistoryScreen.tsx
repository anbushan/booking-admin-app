          ...
          role === "DRIVER" ? (
            <View style={styles.card}>
              <View style={styles.routeRow}>
                ...
                {/* Set only on a ride auto-generated by a recurring
                    series (see recurringRides.routes.js) — distinguishes
                    it from a one-off publish, since it can't be edited
                    or cancelled from here the same way (see Manage
                    recurring rides instead). */}
                {item.recurringTemplateId && (
                  <View style={styles.recurringBadge}>
                    <Ionicons name="repeat" size={10} color={colors.accentText} />
                    <Text style={styles.recurringBadgeText}>{t("history.recurring")}</Text>
                  </View>
                )}
              </View>
...
const styles = StyleSheet.create({
  ...
  recurringBadge: {
    flexDirection: "row", alignItems: "center", gap: 3, backgroundColor: colors.accentBg,
    borderRadius: 999, paddingVertical: 2, paddingHorizontal: 6,
  },
  recurringBadgeText: { ...typography.small, fontSize: 9.5, color: colors.accentText, fontWeight: "700", fontFamily: FONT.bold },
