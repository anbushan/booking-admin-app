...
export default async function DriverDetailPage({ params }: { params: { id: string } }) {
  ...
  const [vehicles, documents, rides, paidBookings] = await Promise.all([
    ...
    prisma.booking.findMany({
      where: { status: "PAID", ride: { driverId: params.id } },
      include: { ride: { select: { pricePerSeat: true } } },
    }),
  ]);

  const totalEarnings = paidBookings.reduce((sum, b) => sum + Number(b.ride.pricePerSeat) * b.seatsBooked, 0);
      ...
      <div style={{ padding: 24, fontFamily: "sans-serif" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginTop: 20 }}>
          ...
          <div style={{ background: "#F1EFE8", borderRadius: 8, padding: 16 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 500 }}>{paidBookings.length}</div>
          </div>
          <div style={{ background: "#F1EFE8", borderRadius: 8, padding: 16 }}>
